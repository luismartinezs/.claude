...
export const EnvSchema = z
  .object({
    ...
    /**
     * Required, never defaulted: development and test switch the paywall off
     * when Stripe is unset, so a deploy that forgot this variable must fail to
     * start rather than quietly run as development.
     */
    ...
    /** Unset outside production: every signed-in account is treated as subscribed. */
    STRIPE_SECRET_KEY: optional,
    STRIPE_WEBHOOK_SECRET: optional,
    /** The one monthly recurring price that Checkout sells (plan key "pro"). */
    STRIPE_PRO_PRICE_ID: optional,
    /**
     * This app's own Customer Portal configuration. The account's default one
     * is shared with every other app on the Stripe account. Optional outside
     * production, where the default is used.
     */
...
export const stripeConfig =
  env.STRIPE_SECRET_KEY !== null && env.STRIPE_WEBHOOK_SECRET !== null && env.STRIPE_PRO_PRICE_ID !== null
    ? {
